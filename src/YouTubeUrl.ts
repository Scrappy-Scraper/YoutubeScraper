export function parseYouTubeUrl(youtubeURL: string): { type: "video" | "channel", id: string, cleanedUrl: string } | null {
...
export function reAdjustYouTubeChannelId(id: string): string {
    if(!channelIdPatterns.some((pattern) => pattern.test(id))) return `channel/${id}`;
    return id;
}
